...
- something ive tried several times and gravitate towards is simulating complex systems like weather, earth, or whatever as a series of very simple systems that layered on top of each other mirror the system. or at least get fairly similar results. each one is understandable meaning the outcome can in general be understood. thats what i thought. given that ive tried it a few times and ive been fairly unsuccessful in implementing - something about my approach, idea, or understanding must be wrong. i think the failure modes have been generally not being able to accurately predict what a simple system will do and it doesnt really have the effect i think it logically should. this takes a lot of time and because i feel so confident that logically such a simple system should work i get stuck in tuning parameters and that sucks all joy and i move on. so something about my approach or if this matches reality must be wrong. 
